...
layer is not found when calling the tools.
    ...
    mkdir build
    cd build
    cmake -DCMAKE_INSTALL_PREFIX=<install-directory> .. (default on Linux: /usr/local, windows: C:/Program Files/ custom: ~/rc_genicam_lib)
    make
    make install
...
-l   List all all available devices on all interfagcces
